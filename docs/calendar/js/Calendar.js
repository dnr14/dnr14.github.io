class App {
  ...
  prevDraw(startObj) {
    ...
    for (let i = 0; i < day; i++) {
      this.$row.append(this.makeSpan(`${prevDate.getDate() + i}`, ["date", "space"]));
    }
  ...
  nextDraw(endObj) {
    ...
    for (let i = 0; i < (6 - endObj.getDay()) + 7; i++) {
      this.$row.append(this.makeSpan(`${nextDate.getDate() + i}`, ["date", "space"]));
    }
  ...
  getToday() {
    ...
    var month = `/0${1 + this.today.getMonth()}`;
